...
Object.extend(RecordSelect.Abstract.prototype, {
  ...
  onkeypress: function(ev) {
    ...
    switch (ev.keyCode) {
      case KEY_UP:
        this.highlight(this.current ? this.current.previous() : this.container.descendants('ol > li').first())
        break;
      case KEY_DOWN:
        this.highlight(this.current ? this.current.next() : this.container.descendants('ol > li').last())
        break;
    }
  },
...
RecordSelect.Autocomplete.prototype = Object.extend(new RecordSelect.Abstract(), {
  onload: function() {
    ...
    // keyboard navigation
    Event.observe(document.body, 'keypress', this.onkeypress.bindAsEventListener(this));
  },
